...
namespace FinancialGrid.Services
{
    ...
    public class FinancialDataService : IFinancialDataService
    {
        ...
        DispatcherTimer _updatePricesTimer;
        List<FinancialRecord> _pricesTimerData;

        DispatcherTimer _updateAllPricesTimer;
        List<FinancialRecord> _allPricesTimerData;

        public FinancialDataService()
        {
            _updatePricesTimer = new DispatcherTimer();
            _updatePricesTimer.Tick += PricesTimer_Tick;

            _updateAllPricesTimer = new DispatcherTimer();
            _updateAllPricesTimer.Tick += AllPricesTimer_Tick;
        }
        ...
        {
            _pricesTimerData = data;
            _updatePricesTimer.Interval = TimeSpan.FromMilliseconds(frequency);
            _updatePricesTimer.Start();
        }

        private void PricesTimer_Tick(object sender, object e)
        {
            UpdatePrices(_pricesTimerData);
        }

        private void AllPricesTimer_Tick(object sender, object e)
        {
            UpdateAllPrices(_allPricesTimerData);
        }

        public void StartUpdatingAllPrices(List<FinancialRecord> data, int frequency)
        {
            _allPricesTimerData = data;
            _updatePricesTimer.Interval = TimeSpan.FromMilliseconds(frequency);
            _updatePricesTimer.Start();
        }
        ...
        Tuple<double, double> GenerateNewPrice(double oldPrice)
        {
